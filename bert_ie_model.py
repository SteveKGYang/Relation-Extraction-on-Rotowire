...
class Bert(nn.Module):
	def __init__(self, load_pretrained_bert, bert_config):
		...
		if(load_pretrained_bert):
			...
			self.model = BertModel.from_pretrained('./data/bert-base-uncased')
...
class IEMODEL(nn.Module):
	def __init__(self, voc_size, emb_size, rnn_input, rnn_hidden, num_layers, rnn_bi, dp, rnn_mlp_size, label_num, enable_cnn, cnn_filter, cnn_kernel_width, load_pretrained_bert, bert_config):
		...
		self.bert_model = Bert(load_pretrained_bert, bert_config)
...
def init_model(args):
	...
	ie_model = IEMODEL(vocab_sizes, emb_sizes, sum(emb_sizes), args.rnn_hidden, 1, True, args.dropout, args.rnn_mlp_hidden, len(labels_dict) + 1, args.cnn, args.cnn_filter, list(map(int, args.cnn_width.strip().split(","))) if args.cnn_width is not None else None, args.load_pretrained_bert, bert_config)
