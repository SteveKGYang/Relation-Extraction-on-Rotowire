...
def train(args, model, tokenizer, NUM_ID, ENT_ID):
	...
	for epoch_no in range(args.epoch):
		...
		for exp_no in range(0, len(h5fi["trsents"]), args.batch_size):
			inputs = []
			words = h5fi["trsents"][exp_no:exp_no+args.batch_size]
			words = words.tolist()
			am_labels = []
			ml_labels = []
			position_ids = []
			for i in range(args.batch_size):
				...
				if i == len(words):
					break
				lines = words[i]
				nums = [i for i, a in enumerate(lines) if a == SEP]
				for j in range(len(nums)-1):
					co = lines[nums[j]+1:nums[j+1]]
					for item in co:
						items.append(item)
				#print(lines)
				#print(tokenizer.convert_ids_to_tokens(lines))
				input_line = lines[:nums[0]]
				am = [1]*len(lines[:nums[0]])
				ml = lines[:nums[0]]
				ct = 0
				for j in range(len(input_line)):
					if input_line[j] == NUM_ID or input_line[j] == ENT_ID:
						input_line[j] = items[ct]
						am[j] = 0
						ct += 1
					if input_line[j] != NUM_ID and input_line[j] != ENT_ID:
						ml[j] = -1
				position_id = [i for i in range(512)]
			...
			'''print(inputs.size())
			print(am_labels.size())
			print(ml_labels.size())
			print(ml_labels)'''
			output = model(inputs, attention_mask=am_labels, masked_lm_labels=ml_labels, position_ids=position_ids)
			loss = output[0]
			if args.gpu:
				loss = loss.cuda()
			# training
...
def main():
	...
	if args.mode == "train":
		global_step, tr_loss = train(args, model, tokenizer, NUM_ID, ENT_ID)
