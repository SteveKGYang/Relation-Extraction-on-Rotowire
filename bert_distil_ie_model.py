...
#from pytorch_transformers import BertModel, BertConfig, BertTokenizer, WarmupLinearSchedule, AdamW
from pytorch_transformers import DistilBertModel, DistilBertConfig,DistilBertTokenizer
...
class Bert(nn.Module):
	def __init__(self, load_pretrained_bert, bert_config):
		...
		if(load_pretrained_bert):
			...
			self.tokenizer = BertTokenizer.from_pretrained('/users3/ywsun/project/data2text/data/bert-base-cased/vocab.txt')
			self.tokenizer.add_tokens([NUM_MASK, ENT_MASK])
			self.model = BertModel.from_pretrained('/users3/ywsun/project/data2text/data/bert-base-cased')
			self.model.resize_token_embeddings(len(self.tokenizer))
		else:
			self.model = BertModel(bert_config)

	def forward(self, words, attn_mask):
		words_output = self.model(words, attention_mask=attn_mask)[0]
		return words_output
'''
...
class IEMODEL(nn.Module):
	def __init__(self, voc_size, emb_size, rnn_input, rnn_hidden, num_layers, rnn_bi, dp, rnn_mlp_size, label_num, enable_cnn, cnn_filter, cnn_kernel_width, load_pretrained_bert, bert_config):
		...
		if enable_cnn:
			Ci = 1
			cnn_mlp_size = word_emb_size + entpos_emb_size + numpos_emb_size
			# cnn_mlp_size = rnn_mlp_size
			Co = cnn_filter
			...
			self.cnn_mlp = nn.Sequential(
					nn.Linear(len(cnn_kernel_width) * Co, cnn_mlp_size),
					nn.ReLU(),
					nn.Dropout(dp)
				)		
		...
		self.rnn_mlp = nn.Sequential(
				...
				nn.Linear(rnn_hidden, rnn_mlp_size),
				nn.ReLU(),
				nn.Dropout(dp)
			)
		...
		if self.enable_cnn:
			...
			decoder_input = rnn_mlp_size + cnn_mlp_size
		else:
	...
	def forward(self, x, lengths, attn_mask):
		...
		word_bert = self.bert_model(word, attn_mask)
		word_bert = word_bert[0]
		# word = self.word_emb(word)
		entpos = self.entpos_emb(entpos)
		numpos = self.numpos_emb(numpos)
		assert word_bert.size(1) == entpos.size(1) and word_bert.size(1) == numpos.size(1)
		combined = torch.cat((word_bert, entpos, numpos), 2)
		
		if self.enable_cnn:
			cnn_input = combined.unsqueeze(1)
			# cnn_input = word_bert.unsqueeze(1)
		...
		memory_bank, _ = self.rnn(packed_emb)
...
parser.add_argument('-bert_config', type=str, default=None,
...
def init_model(args):
	emb_sizes = (BERT_SIZE, args.word_emb_size // 2, args.word_emb_size // 2)
	# train
	param_init = 0.1
	...
	if args.bert_config is not None:
		bert_config = DistilBertConfig.from_json_file(args.bert_config)
	else:
		bert_config = None
	'''
...
def train(args, ie_model):
	...
	# optim = AdamW(ie_model.parameters(), lr=2e-5, correct_bias=False)
...
def test(args, ie_model):
	...
	for exp_no in range(0, len(h5fi["valsents"]), args.batch_size):
		words = torch.from_numpy(h5fi["valsents"][exp_no:exp_no+args.batch_size]).long()
		entpos = torch.from_numpy(h5fi["valentdists"][exp_no:exp_no+args.batch_size]).long() + 200
		numpos = torch.from_numpy(h5fi["valnumdists"][exp_no:exp_no+args.batch_size]).long() + 200
		labels = torch.from_numpy(h5fi["vallabels"][exp_no:exp_no+args.batch_size]).long()
