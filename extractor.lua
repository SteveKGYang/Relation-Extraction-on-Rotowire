...
cmd:option('-test', true, [[use test data]])
...
function get_single_acc(model, valbatches, ignoreIdx, convens, lstmens)
    if not g_maxes then
        g_maxes = torch.CudaTensor()
        g_argmaxes = torch.CudaLongTensor()
...
function set_up_saved_models()
 --[[
  ...
  local lstmens_paths = {"blstmie-ep7-1.t7",
			 ...
			 "blstmie-ep10-3.t7"}
  --]]
  local convens_paths = {"models/conv1ie-ep6-94-74.t7",
                         "models/conv2ie-ep3-94-60.t7",
			 "models/conv3ie-ep8-95-72.t7"}

  local lstmens_paths = {"models/blstm1ie-ep4-93-75.t7",
	                 "models/blstm2ie-ep3-93-71.t7",
			 "models/blstm3ie-ep2-94-72.t7"}  
  opt.embed_size = 200
  opt.num_filters = 200
...
function main()
    ...
    for i = 1, opt.epochs do
        ...
        local acc, rec = get_single_acc(model, valbatches, opt.ignore_idx)
        print("acc:", acc)
